...
if not os.path.exists(FILE_NAME):
    ...
    st.write("Make sure the Excel file is uploaded in the same folder as app.py in GitHub.")
    st.stop()
...
def load_data():
    ...
    for sheet in SHEETS:
        try:
            ...
            required_cols = ["Meter No", "Outage Date Time", "Restore Date Time"]
...
if len(data_dict) == 0:
    st.error("No valid sheets loaded. Please check your Excel file.")
    st.stop()
...
meter_list = sorted(df["Meter No"].dropna().unique())
